...
function update(title,url, id,width,height) {
	...
	if (rowsData.length>1) {
		$.messager.alert('提示','请选择一条记录再编辑!','info'); 
		//tip('请选择一条记录再编辑');
		return;
	}
	var idField = $('#'+id).datagrid('options').idField;
	url += '&id='+ rowsData[0][idField];
...
//	}
	...
	if (!rowsData || rowsData.length == 0) {
		$.messager.alert('提示','请选择查看项目!','info');
		return;
	}
	if (rowsData.length > 1) {
		$.messager.alert('提示','请选择一条记录再查看!','info');
		return;
	}
	
	var idField = $('#'+id).datagrid('options').idField;
    url += '&load=detail&id='+rowsData[0][idField];
 ...
 * @param saveurl
 */
function createdetailwindow(title, addurl,width,height,authType) {
 ...
 * @param saveurl
 */
function createnobtnwindow(title, addurl,width,height,authType) {
	if(!checkAuth(authType)){
		return;
	}
	width = width?width:700;
...
function checkAuth(authType){
	...
	if(authType != null && authType != ""){
		$.ajax({
			...
			cache : false,
			type : 'POST',
			url : "commonController.do?checkAuthority",// 请求的action路径
			data : {
				authValue : authType
			},
		...
		returnVal = true;
	if(!returnVal)
		$.messager.alert("提示","当前作业点没有权限","error");
	return returnVal;
}
...
function editfs(title,url) {
	var name=gridname;
	 if (rowid == '') {
		$.messager.alert('提示','请选择编辑项目!','info');
		return;
	}
	url += '&id='+rowid;
